...
const socketDirectory = "/var/run/wireguard"

func uapiPath(iface string) string {
	return fmt.Sprintf("%s/%s.sock", socketDirectory, iface)
}

func uapiDial(iface string) (net.Conn, error) {
	return net.Dial("unix", uapiPath(iface))
}

// readUAPIResponse reads until a blank line (end of UAPI response).
